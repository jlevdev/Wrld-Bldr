...
const InfoHeader = () => {
    const { activeShop, activeNPC, setActiveNPC, updateActiveShopCash } = usePaper();
        ...
        <Styled.Container>
            <Styled.ShopContainer>
                <Heading size={3}>
                    ...
                    <Styled.ShopFinances>
                        <i>gold &nbsp;</i>
                        <Styled.CoinInput onChange={handleInputChange} className="gold-counter" type="number" defaultValue={activeShop.cash.gold} />

                        <i>silver &nbsp;</i>
                        <Styled.CoinInput onChange={handleInputChange} className="silver-counter" type="number" defaultValue={activeShop.cash.silver} />

                        <i>copper &nbsp;</i>
                        <Styled.CoinInput onChange={handleInputChange} className="copper-counter" type="number" defaultValue={activeShop.cash.copper} />
                    </Styled.ShopFinances>
                </Heading>
            </Styled.ShopContainer>
            <Styled.NPCPortraits>
                {
                    activeShop.npcs.map((n, i) => {
                        return <span onClick={() => { setActiveNPC(n) }} key={i} style={{ width: 40, height: 40, backgroundImage: `url(${n.avatar})`, backgroundRepeat: 'no-repeat', backgroundSize: 'cover', backgroundPosition: 'center' }}>
                        </span>
                    })
                }
            </Styled.NPCPortraits>
