                    ...
                    <Styled.InventoryList>
                        {
                            this.props.activeShop.inventory.map((t, index) => {
                                if (t.type == Item.SERVICE_TYPE) {
                                    return <li className="inv-service" key={index}>{t.name} - {t.cost}</li>
                                } else {
                                    if (typeof t.rarity === 'object') {
                                        return <Styled.InventoryListItem style={{ color: t.rarity['color'] }} key={index}>
                                            <Styled.ItemCounter className="inv-counter"> {t.amount} </Styled.ItemCounter>
                                            {t.name} - {t.cost}
                                            <Styled.ItemControls className="inv-controls">
                                                <Styled.ItemControlsPlus onClick={() => { addItem(t) }} className="inv-plus"></Styled.ItemControlsPlus>
                                                <Styled.ItemControlsMinus onClick={() => { removeItem(t) }} className="inv-minus"></Styled.ItemControlsMinus>
                                            </Styled.ItemControls></Styled.InventoryListItem>
                                    } else {
                                        return <Styled.InventoryListItem key={index}>
                                            <Styled.ItemCounter className="inv-counter"> {t.amount} </Styled.ItemCounter>
                                            {t.name} - {t.cost}
                                            <Styled.ItemControls className="inv-controls">
                                                <Styled.ItemControlsPlus onClick={() => { addItem(t) }} className="inv-plus"></Styled.ItemControlsPlus>
