...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for f in glob(
            path.join(settings.BASE_DIR, "initial_files\\avatars") + "\\*.png"
        ):
        ...
        # generate wallpapers
        for f in glob(
            path.join(settings.BASE_DIR, "initial_files\\backgrounds") + "\\*.jpg"
        ):
            ...
            print(response.text)
        self.stdout.write(self.style.SUCCESS("Successfully generated Backgrounds"))

        # generate icons
        for f in glob(path.join(settings.BASE_DIR, "initial_files\\icons") + "\\*.png"):
        ...
        Settlement.objects.create_with_random_shops(
            name="Thunder Bluff",
            map_data={"seed": 1, "size": 40, "shopIndexes": [1, 2, 3, 4, 5]},
            owner_id=2,
        )
