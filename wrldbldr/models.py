...
class Icon(models.Model):
    created_at = models.DateTimeField(auto_now_add=True)
    name = models.CharField(max_length=200, default="", primary_key=True, unique=True)
...
class Item(models.Model):
    ...
    location_type = models.ForeignKey(
        "Location_Type", on_delete=models.PROTECT, default=None, null=True
    )
    location = models.ForeignKey(
        "Location", on_delete=models.PROTECT, default=None, null=True
    )
    rarity = models.ForeignKey(
        "Rarity", on_delete=models.PROTECT, default=None, null=True
    )
...
class Location_Manager(models.Manager):
    def create_shop(self, settlement, shop_type, shop_index, name="", description=""):
        if name == "":
            # generate name here
        ...
        inventory_size = random.randint(
            inventory_size_range["low"], inventory_size_range["high"]
        )
        item_options = (
            Item.objects.filter(is_template=True)
            .order_by("?")
            .filter(location_type_id=shop_type.id)[:inventory_size]
        )
        ...
        for i in item_options:
            new_item = i
            new_item.pk = None
            new_item.quantity = random.randint(
                new_item.quantity_low, new_item.quantity_high
            )
        ...
        starting_coin_ranges = Constants.SHOP_STARTING_COINS[shop.fiscal_status]
        shop.gold = random.randint(
            starting_coin_ranges[0][0], starting_coin_ranges[0][1]
        )
        shop.silver = random.randint(
            starting_coin_ranges[1][0], starting_coin_ranges[1][1]
        )
        shop.copper = random.randint(
            starting_coin_ranges[2][0], starting_coin_ranges[2][1]
        )
        shop.save()
...
class Location(models.Model):
    ...
    settlement = models.ForeignKey(
        "Settlement", on_delete=models.CASCADE, default=None, null=True
    )
    location_type = models.ForeignKey(
        "Location_Type", on_delete=models.PROTECT, default=None, null=True
    )
...
class Location_Type(models.Model):
    id = models.AutoField(primary_key=True)
    name = models.CharField(max_length=200, default="No Name Given", unique=True)
...
class NPC(models.Model):
    ...
    avatar = models.ForeignKey(
        "Avatar", on_delete=models.PROTECT, default=None, null=True
    )
    created_at = models.DateTimeField(auto_now_add=True)
    description = models.TextField(default="", blank=True)
    name = models.CharField(max_length=200, default="No Name Given")
...
class SettlementManager(models.Manager):
    def create_with_random_shops(self, name, map_data, owner_id):
        settlement = Settlement(
            name=name, map_data=json.dumps(map_data), owner_id=owner_id
        )
        settlement.save()
        ...
        for s in shops:
            ...
            Location.objects.create_shop(
                settlement=settlement, shop_type=shop_types.pop(0), shop_index=s
            )
...
class Settlement(models.Model):
    ...
    clone_of = models.ForeignKey(
        "Settlement", on_delete=models.DO_NOTHING, default=None, null=True
    )
    created_at = models.DateTimeField(auto_now_add=True)
    """
        map_data={
    ...
    owner = models.ForeignKey(
        "auth.User", on_delete=models.CASCADE, default=None, null=True
    )
